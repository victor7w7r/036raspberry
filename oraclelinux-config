...
function hostnamer {
    ...
    --inputbox "Please write your hostname (ex: A036-oracle)" 8 80 2>"$HOSTTEMP"
...
function graphical {
	...
	dialog --title "Graphical" --backtitle "036 Creative Studios" \
		--yesno "Install XFCE as Desktop Environment?" 7 60
...
function ohmyzsh {
	...
	for HOME in $LISTHOME; do
		...
		echo 'sudo ln -s "$HOME"/.zshrc /root/.zshrc' >> /home/"$HOME"/omz.sh
		echo 'sudo ln -s "$HOME"/.oh-my-zsh /root/.oh-my-zsh' >> /home/"$HOME"/omz.sh
